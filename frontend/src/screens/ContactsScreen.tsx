...
export default function ContactsScreen({
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const remote = await api.get(`/contacts`, { query: { user_id: userId } });
        if (Array.isArray(remote) && remote.length > 0) {
          // Map remote shape to local contact shape
        ...
        // ignore and keep local contacts
      }
    })();
  }, []);
  ...
  const handleAdd = () => {
    ...
    (async () => {
      try {
        const userId = localStorage.getItem("resqai_user_id") || "demo-user";
        await api.post(`/contacts`, { body: { user_id: userId, name: newContact.name, phone: newContact.primaryPhone, relation: "" } });
      } catch (err) {
  ...
  const handleDelete = (id: number) => {
    ...
    (async () => {
      try {
        const userId = localStorage.getItem("resqai_user_id") || "demo-user";
        await api.del(`/contacts/${id}`, { query: { user_id: userId } });
      } catch (err) {
        // ignore
      }
    })();
