...
export default function AIClassificationScreen({ isDark, onNavigate }: AIClassificationProps) {
  ...
  const startAnalysis = () => {
    ...
    // Call backend triage streaming endpoint if available
    (async () => {
      try {
        const userId = localStorage.getItem("resqai_user_id") || "demo-user";
        const lat = 40.7128;
        const lng = -74.0060;

        const res = await fetch(`${(import.meta.env.VITE_API_BASE_URL || "http://127.0.0.1:8000/api")}/triage`, {
          method: "POST",
          headers: { "Content-Type": "application/json", Accept: "text/event-stream" },
          body: JSON.stringify({ description: inputText || selectedPrompt, lat, lng }),
        });
          ...
          if (value) {
            ...
            for (const part of parts) {
              ...
              if (line) {
                ...
                // we could parse JSON, but backend yields plain events
                prog = Math.min(100, prog + 10 + Math.random() * 20);
              ...
              <motion.button
                ...
                onClick={async () => {
                  try {
                    ...
                    const lat = 40.7128;
                    const lng = -74.0060;
                    await api.post(`/sos`, { body: { lat, lng, user_id: userId, severity: classification } });
                  } catch (e) {
                    // ignore
                  }
