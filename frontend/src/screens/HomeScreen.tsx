...
export default function HomeScreen({
  ...
  useEffect(() => {
    ...
    // Probe backend health and try to fetch nearby services (falls back to mock)
    (async () => {
      try {
        await api.get("/health");
        // Optionally fetch remote services if backend exposes them; placeholder
        // const services = await api.get('/facilities/nearby', { query: { lat: 40.7128, lng: -74.006 } });
        // if (Array.isArray(services)) { /* update local state when wiring fully */ }
      } catch (e) {
        // ignore - offline or backend not running
      }
    })();
